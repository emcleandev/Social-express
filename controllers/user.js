...
const { sendVerification, sendResetCode } = require("../helpers/mailer");
...
exports.register = async (req, res) => {
  try {
    ...
    res.send({
      ...
      message: "Register Success | Please activate your email to begin",
    });
  } catch (error) {
...
exports.activateAccount = async (req, res) => {
  try {
    ...
    if (validUser !== user.id) {
      return res.status(400).json({ message: "You don't have the authority" });
    }
  ...
  try {
    if (req.user.id !== req.params.id) {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ message: "You can't double follow you stalker." });
      }
    } else {
  ...
  try {
    if (req.user.id !== req.params.id) {
      ...
      if (
        ...
        res.json({ message: "Friend is now not a friend :(" });
      } else {
